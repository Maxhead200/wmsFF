...
export function ClientCabinetFilters({ value, totals, isOpen, onChange, onToggle }: ClientCabinetFiltersProps) {
  ...
  return (
    <section
      className={`client-cabinet-filters ${isOpen ? 'is-open' : 'is-collapsed'}`}
      aria-label="Фильтры клиентского кабинета"
    >
      <div className="client-cabinet-filters__header">
        <button
          className="client-cabinet-filters__toggle"
          type="button"
          onClick={onToggle}
          aria-expanded={isOpen}
          title={isOpen ? 'Свернуть фильтры' : 'Показать фильтры'}
        >
          ...
          <ChevronDown className="client-cabinet-filters__chevron" size={17} aria-hidden="true" />
        </button>
