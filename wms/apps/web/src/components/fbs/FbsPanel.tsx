...
export function FbsPanel({ session, onOpenRequest }: FbsPanelProps) {
  ...
  async function deliverSelectedSupplies(orders: FbsOrderSummary[]) {
    ...
    try {
      // ADDED: load live WB destinations before asking the manager to confirm.
      const payload = {
        clientId: selectedClientId,
        orders: orders.map((order) => ({ connectionId: order.connectionId, id: order.id })),
      };
  ...
  async function submitFbsSupplyDelivery() {
    if (!selectedClientId || !deliveryDialog || orderAction !== null) return;
    if (!deliveryDialog.destinationOfficeId || !deliveryDialog.plannedDeliveryDate) {
    ...
    <section className="fbs-panel" aria-label="FBS">
      ...
      {deliveryDialog ? (
        <FbsSupplyDeliveryDialog
          ...
          onSubmit={() => void submitFbsSupplyDelivery()}
        />
      ) : null}
    </section>
  );
...
export function canSubmitFbsSupplyDelivery(
  ...
  // TEST: keep the irreversible button disabled until WB destination and the
  // local delivery date are both confirmed.
  return Boolean(
