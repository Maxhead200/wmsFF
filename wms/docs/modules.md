...
- Web-панель волн сборки открывает печатный лист волны: до запуска показывает плановые короба из `AVAILABLE`, после запуска - фактические аллокации из результата сборки.
...
- Печатный HTML-лист волны показывает маршрут сборки по строкам заявок: клиент, SKU, штрихкод, количество, чекбокс сборщика и короба/паллеты.
- Web-панель склада показывает кандидатов на волну, создает волну, запускает последние волны и открывает лист сборки из отдельного блока `Волны сборки`.
