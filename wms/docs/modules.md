...
- Оператор может открыть складскую инструкцию по outbound-заявке до запуска сборки: документ показывает маршрут по коробам/паллетам, полный или частичный отбор и нехватку.
