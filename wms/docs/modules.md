...
- `GET /api/v1/billing/invoices/:id/document` и `/document.pdf` - печатный HTML/PDF-документ счета с позициями, оплатами и проверкой client scope на чтение.
- `GET /api/v1/billing/invoices/:id/act` и `/act.pdf` - печатный HTML/PDF-акт оказанных услуг на основе снимка позиций счета.
