...
- В таблице заявок оператор может запустить сборку outbound-заявки; система подбирает доступные остатки, переводит их в упаковку и сохраняет движения в stock ledger.
...
- Упаковка, отгрузка и batch picking поверх уже созданного слоя `PICK -> PACKING`.
