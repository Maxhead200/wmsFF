...
- Android-ТСД хранит pending/rejected операции в Room, умеет повторять pending batch и показывает оператору отклоненные операции.

## Следующие этапы

- Расширение клиентских scope на личный кабинет и заявки клиента.
- ТСД: экран входа, выдача device-token, сценарии receipt/inventory и конфликтная очередь разбора расхождений.
